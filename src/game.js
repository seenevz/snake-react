...
class SnakeGame {
    ...
    this.snake.forEach(snakeComponent => {
      if (this.directionStack.length > 0) {
        this.directionStack.forEach(directionObj => {
          if (
            snakeComponent.x === directionObj.x &&
            snakeComponent.y === directionObj.y
          ) {
            snakeComponent.changeDirection(directionObj.direction);
            ++directionObj.counter;
          }
          if (directionObj.counter === this.snake.length) {
  ...
  checkSnakeCollision = () => {
    ...
    snakeBody.forEach(snakeComponent =>
      snakeHead.checkCollision(snakeComponent, this.stopGame)
    );
